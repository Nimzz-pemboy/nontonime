              ...
              <div key={format.title} className="space-y-2">
                ...
                {format.qualities.map((quality) => (
                  <div key={quality.title} className="flex flex-wrap items-center gap-1.5">
                    <span className="text-xs font-semibold text-popover-foreground">{quality.title}</span>
                    {quality.urls.map((link) => (
                      <a
                        ...
                        className="rounded-full border border-border px-2 py-1 text-[11px] font-semibold text-popover-foreground transition-colors hover:bg-accent"
                      >
                        {link.title}
                      </a>
                    ))}
